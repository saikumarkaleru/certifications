...
```mermaid
flowchart TD
    A["Least vulnerable lender - least regulation"] --> B["Banks and PFIs and Government - fully exempt they regulate themselves"]
    B --> C["Directors and insiders - exempt with a written non-borrowed declaration"]
    C --> D["Members of the company - light lane Section 73 with conditions"]
    D --> E["General public - heaviest lane Section 76 conditions plus rating"]
    E --> F["Most vulnerable lender - most regulation"]
```
...
```mermaid
flowchart TD
    A["Money received by the company"] --> B{"Is the source on the exempt list Rule 2 1 c"}
    B -->|"No - not on list"| C["It is a DEPOSIT - apply Sec 73 to 76A"]
    B -->|"Yes - on list"| D{"Are all conditions of that exemption satisfied"}
    D -->|"Yes"| E["NOT a deposit"]
    D -->|"No - condition breached or clock expired"| F["Converts to a DEPOSIT from the date of breach"]
```
