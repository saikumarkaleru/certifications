...
> **Why this chapter is a scoring goldmine.** Unlike ratio analysis or cash-flow (where a single silly slip cascades), amalgamation is *modular*: PC is one working, net assets another, journals another, balance sheet another. Even if one working is wrong, the examiner awards the others on their own merits. The student who *shows every working note separately* and *labels the method* harvests marks the student who writes one blurred solution loses.
...
**A second analogy for the two PC methods.** Imagine buying a used car. You can price it *from the outside* — "I'll hand you ₹4 lakh cash plus my old bike" (that is **Net Payments**: add up what you give the *seller*). Or you can price it *from the inside* — "the engine is worth this, the tyres that, minus the loan still on it" (that is **Net Assets**: value what you *receive*, net of debts you assume). Both are legitimate ways to arrive at a price, but they answer different questions, and — crucially — the *gap* between the two is exactly what surfaces as **goodwill or capital reserve**. When the question hands you one method's inputs, that is the method it wants.
...
> **A finer legal note for MCQs.** *Amalgamation* in the loose textbook sense (new company formed) is sometimes called *"amalgamation in the narrow sense"* or *consolidation of companies*. Under the Companies Act 2013 (Sections 230–232), both amalgamation and absorption proceed through a **scheme of arrangement sanctioned by the National Company Law Tribunal (NCLT)**. AS 14 does *not* care about the legal label — it cares only about the *substance test*. So an "absorption" can still be *in the nature of merger* if all five conditions hold. Do not tie "merger accounting" to "new company formed" — that is a classic MCQ trap; the two are independent.
...
> **Why 90%, why "wholly equity", why "same business"?** Each condition is a *proxy* for economic continuity. The **90% equity threshold** ensures the *same owners* continue — if a big slice cashed out, the group has changed, so it is a purchase. The **wholly-equity consideration** rule ensures no one was *bought out for money* — cash is the fingerprint of a sale. The **"business carried on"** rule ensures the acquirer is not just stripping assets. The **"book values unchanged"** rule ensures no one is marking up the balance sheet to manufacture reserves. Strip away the legalese and every condition is asking one question: *did the ownership and the enterprise genuinely continue, or did one party sell to another?*
...
> **A word on Ind AS, for context only (not examinable at CA Inter under AS).** Under **Ind AS 103 (Business Combinations)** the *pooling method is abolished for third-party combinations* — almost everything is an "acquisition" measured at fair value, and pooling survives only for *common control* transactions. CA Intermediate is examined under **AS 14**, where both methods are alive. If a question cites AS 14, use the five-condition test; do not import Ind AS 103 rules. *Flag: confirm the applicable framework in your current ICAI study material.*
...
> **Micro-drills on the conditions (each is a favourite one-mark trap):**
> - The **90%** is measured on the shares held by *outsiders* — you *exclude* shares the transferee already holds in the transferor before computing the ratio. So if the transferee already owns 20% of the transferor and the remaining 80% outsiders all take equity shares, condition 2 is *satisfied* (100% of the outsider block converts), even though only 80% of the total converts.
> - Condition 3 refers to consideration to **equity** shareholders. The transferee can pay **preference** shareholders in cash or preference shares without breaking merger status — condition 3 only polices the *equity* leg. (Verify the exact wording in current ICAI material; ICAI's phrasing centres on the equity consideration.)
> - Condition 5 permits changes for **uniformity of accounting policies** (e.g. aligning depreciation method). That is *not* a "revaluation" — do not fail a merger just because depreciation was re-aligned.
...
- **Watch the word "liability".** Under Net Assets, *only third-party liabilities* (debentures, creditors, provisions, outstanding expenses) are deducted. Share capital and reserves are **NOT** deducted — they are the *owners' claim*, i.e. exactly the net assets you are trying to measure. Deducting them double-counts and zeroes out your PC.
...
#### (c) Two more PC computation styles the exam sneaks in

Beyond the two named methods, examiners disguise PC three ways:

1. **Lump-sum method.** The question simply states "PC is ₹X". Nothing to compute — but you still must *discharge* it correctly (allocate between shares, premium, cash).
2. **Intrinsic value / share-exchange method.** The question gives the **intrinsic (net asset) value per share** of each company and an **exchange ratio** (e.g. "3 shares of transferee for every 5 of transferor"). Here PC = number of shares issued × issue value, where the *number* comes from the exchange ratio. This is really Net Payments dressed up — the ratio just tells you *how many* shares change hands.
   ```
   Intrinsic value per share = Net assets available to that class ÷ number of shares
   Exchange ratio = Transferor's intrinsic value per share ÷ Transferee's intrinsic value per share
   Shares to issue = Transferor's shares × exchange ratio
   ```
3. **Payments-in-kind.** Consideration discharged partly by handing over *an asset* (e.g. investments) rather than cash/shares — value that asset at agreed value and include it in Net Payments.

> **Reconciling the two named methods.** In a **purchase**, Net Assets and Net Payments will usually give *different* numbers — the difference is goodwill/capital reserve, which is the whole point. In a **genuine merger at par with no revaluation**, they *coincide*. If a question lets you compute both and they differ, you have *not* made an error — you have discovered the goodwill. Only pick the method the deal *describes* as the PC; the other becomes a cross-check on net assets.
...
> **The sign, from first principles.** "Net assets" is the *fair worth* of what you received. PC is *what you paid*. Pay **more than worth** → you believe in unrecorded value (reputation, customers) → capitalise it as **Goodwill** (an asset). Pay **less than worth** → you got a bargain → the windfall is a **capital profit**, parked in **Capital Reserve** (never P&L — it is not a trading gain, and it is not distributable as dividend). Getting this backwards is an instant zero, so anchor it: *"paid more, got goodwill; paid less, got a reserve."*

> **Edge case — goodwill AND capital reserve in the same problem.** When a single transferee absorbs **two transferors**, and one is a premium purchase (goodwill) and the other a bargain (capital reserve), AS 14 does **not** permit netting one against the other — show **Goodwill** on the asset side for the first and **Capital Reserve** on the equity side for the second. (For a *single* transferor, only one of the two can arise.)
...
> **The premium is a discharge-side event, not a PC-side event.** If PC (Net Payments) already values shares at issue price (₹12), do *not* add the premium again on discharge — you split the *same* ₹12 into ₹10 capital + ₹2 premium. Double-counting the premium (once in PC, once on top) is a silent balancing error that examiners plant deliberately. The rule: **PC = capital portion + premium portion**, not PC + premium.

> **Discharging by debentures issued to shareholders.** If shareholders (not the old debenture-holders) receive the transferee's debentures as part of their consideration, those debentures **are** part of PC. Contrast with debentures issued to *take over the transferor's existing debenture-holders* — those are **outside** PC. Same instrument, opposite treatment, decided purely by *who receives it*.
...
> **Two subtleties in the transferor's books that decide 3–4 marks:**
> - **Fictitious assets** (preliminary expenses, discount on issue, debit balance of P&L, underwriting commission) are *never* transferred to Realisation. They are written off directly against the Equity Shareholders' Account — they were never real value. Route them to Realisation and your Realisation profit is wrong.
> - **What figure goes to the Preference Shareholders' Account?** Preference shareholders are settled *at the agreed amount they receive*, which may be **above or below** their paid-up capital. Any excess paid is a *loss on realisation* borne by equity; any shortfall is a *gain*. The book value of preference capital goes to *their* account; the difference flows through Realisation.

**How Realisation profit/loss arises — the intuition.** The Realisation Account debits assets at *book value* and credits (a) liabilities taken over at book value and (b) the PC received. If the transferee pays **more than the book net worth of assets it took**, the transferor's shareholders make a **realisation profit**; if less, a **realisation loss**. So the transferor's profit mirrors — in reverse sign relative to book values — the transferee's goodwill/capital reserve (which is measured against *agreed* values, not book). They are two views of the same bargain.
...
> **Why "adjust in reserves" and never goodwill under Pooling.** In a merger nobody bought anybody, so by definition there is no premium to capitalise. But the *arithmetic* can still leave a gap: the transferee might issue ₹8,00,000 of its own equity to replace the transferor's ₹7,00,000 equity. That extra ₹1,00,000 of capital has to come from *somewhere* on the credit side — it is taken *out of the pooled reserves* (reduce General Reserve / Revenue Reserve first, then Capital Reserve). Conversely, if the transferee issues *less* capital than the transferor had, reserves go *up*. The balance sheet stays honest without ever inventing an asset. **Order of adjustment matters in strict solutions: hit revenue/free reserves before capital reserves.**

### 4.7 Treatment of specific items (both methods)

- **Statutory reserves** (e.g. reserves required by law, such as an Investment Allowance Reserve or a statutory Development Rebate Reserve): under Purchase Method they are recorded via an **Amalgamation Adjustment Reserve** (shown as a *separate line under "Reserves & Surplus" with a negative sign*, i.e. contra to the statutory reserve) so the statutory reserve survives; it is reversed when the statutory requirement lapses. Under Pooling, statutory reserves carry over naturally.
...
- **Debentures of the transferor.** If taken over, the transferee usually *issues its own debentures in exchange*. These may be issued at par, premium, or discount and are **not** part of PC. If the transferor's debenture-holders are paid at a premium, that premium is a cost the transferee bears (often to capital reserve/goodwill or as a loss on realisation in the transferor's books, depending on who bears it).
- **Dissenting shareholders / non-converting minority.** If under 90% of equity holders take shares, not only does merger status fail — the dissenters must be *paid out in cash*, which further confirms purchase treatment and reduces the equity-share count issued.

```mermaid
flowchart TD
    A["Amount the transferee agrees to pay"] --> B{"Who is the recipient"}
    B -->|"Equity or preference SHAREHOLDERS of transferor"| C["Part of Purchase Consideration"]
    B -->|"Debenture-holders being taken over"| D["Outside PC - settled by issuing debentures or cash"]
    B -->|"Creditors and other third parties"| E["Outside PC - taken over as liabilities"]
    B -->|"Liquidation or formation expenses"| F["Outside PC - charged per policy"]
    C --> G["Discharge in shares premium and cash"]
```
*Figure 2 — The recipient test: only payments to shareholders are purchase consideration.*
...
> **Examiner tweak — "what if goodwill is retained at ₹40,000?"** Then Net assets = 6,60,000 + 40,000 = 7,00,000, so goodwill on amalgamation = 7,20,000 − 7,00,000 = ₹20,000. You would then hold *two* goodwills — the ₹40,000 old goodwill (now an acquired asset) plus ₹20,000 fresh — totalling ₹60,000 on the asset side. The balance sheet is identical in total; only the *label split* changes. Read whether old goodwill is "taken over" or "valued at nil".

> **Examiner tweak — "shares issued at par instead of ₹12."** PC = 60,000 × ₹10 = ₹6,00,000. Now PC (6,00,000) < Net assets (6,60,000) → **Capital Reserve ₹60,000**. Same facts, one changed term, and goodwill *flips* to a capital reserve. This is exactly why you must recompute PC from the *stated* issue price every time.
...
- Net assets (agreed): Fixed assets 9,80,000 + Stock 2,40,000 + Debtors 1,60,000 + Bank 20,000 − Creditors 2,00,000 = **₹12,00,000** (i.e. 9,80,000+2,40,000+1,60,000+20,000 = 14,00,000; less 2,00,000 = ₹12,00,000).
...
> **Reconcile the two cases on total equity.** Case A equity side: Equity capital 8,00,000 + Gen. Reserve 2,00,000 + P&L 1,00,000 = **₹11,00,000** of shareholders' funds. Case B: Equity capital 8,00,000 + Securities Premium 1,60,000 + Capital Reserve 2,40,000 = **₹12,00,000**. The ₹1,00,000 difference is precisely the ₹80,000 asset revaluation write-up (9,80,000 − 9,00,000) net of... actually the write-up is ₹80,000; the remaining ₹20,000 gap is the premium arithmetic — the point for the exam is not the reconciliation to the rupee but that *revaluing up inflates the transferee's net worth*, which is exactly why AS 14 blocks revaluation in a genuine merger.
...
> **Examiner tweak — "shares issued at ₹12, not par."** Recompute PC: A = 2,40,000 × 12 + 5,00,000 = ₹33,80,000; B = 1,50,000 × 12 + 3,00,000 = ₹21,00,000. Now PC(A) 33,80,000 vs NA 33,00,000 → **Goodwill ₹80,000**; PC(B) 21,00,000 vs NA 19,00,000 → **Goodwill ₹2,00,000**. Both flip to goodwill, and you would show **Goodwill ₹2,80,000** on the asset side and a **Securities Premium** of ₹9,80,000 (3,90,000 × ₹2) on the equity side. Notice: the *same* balance sheet total moves, and goodwill and capital reserve are mirror-images of the issue price. Always drive PC off the stated issue price.

---

### Example 4 — External Reconstruction (the sick-company rescue)

*Gamma Ltd's balance sheet is crippled by accumulated losses. A new company, New Gamma Ltd, is floated to take it over and start clean.*

| Liabilities | ₹ | Assets | ₹ |
|---|---:|---|---:|
| Equity capital (1,00,000 × ₹10) | 10,00,000 | Land & Building | 4,00,000 |
| 8% Debentures | 3,00,000 | Plant & Machinery | 5,00,000 |
| Creditors | 2,00,000 | Stock | 1,50,000 |
| | | Debtors | 1,20,000 |
| | | Bank | 30,000 |
...
**Terms:**
1. New Gamma Ltd takes over all assets (except the debit P&L, which is a fictitious asset) and all liabilities.
2. Assets revalued: Land & Building ₹4,50,000, Plant ₹3,50,000, Stock ₹1,20,000, Debtors ₹1,00,000 (₹20,000 bad), Bank at book.
3. Equity shareholders receive **1 equity share of ₹10 (at par) for every 2 shares held** — i.e. 50,000 shares. Debentures and creditors taken over at book value.

**Step 1 — Method.** Assets revalued → condition 5 fails → **Purchase Method** (external reconstruction is always accounted as a purchase-style amalgamation).

**Step 2 — PC (Net Payments).** 50,000 shares × ₹10 = **₹5,00,000** (equity only; note the *fresh start* — shareholders accept far fewer shares because the company is sick).
...
| *Less:* 8% Debentures | (3,00,000) |
| *Less:* Creditors | (2,00,000) |
| **Net assets** | **5,50,000** |

**Step 4 — Goodwill/Capital Reserve:** PC 5,00,000 − NA 5,50,000 = **Capital Reserve ₹50,000**.

Notice the **debit P&L of ₹3,00,000 is NOT taken over** — it is a fictitious asset that dies with the old company (written off against the old shareholders in Gamma's Realisation/Equity Shareholders account). This is the *whole point* of external reconstruction: the accumulated loss is scrubbed off the new balance sheet.

**New Gamma Ltd — incorporation entry:**

| Particulars | Dr (₹) | Cr (₹) |
|---|---:|---:|
| Land & Building A/c | 4,50,000 | |
| Plant & Machinery A/c | 3,50,000 | |
| Stock A/c | 1,20,000 | |
...
*Self-check:* Dr = 4,50,000+3,50,000+1,20,000+1,00,000+30,000 = ₹10,50,000. Cr = 3,00,000+2,00,000+5,00,000+50,000 = ₹10,50,000. ✓

**The lesson:** external reconstruction is mechanically identical to any purchase amalgamation, with two signatures — (a) the *new* company usually has the *same shareholders and business*, and (b) the *accumulated loss / fictitious assets are deliberately left behind* to clean the balance sheet. Contrast this with **internal reconstruction** (next: no new company, capital simply reduced within the same entity).

---

### Example 5 — Cross-holding: transferee already owns shares in the transferor

*S Ltd (₹10 shares) has 2,00,000 equity shares. H Ltd already owns 40,000 of them (20%). H Ltd absorbs S Ltd, issuing 3 of its own ₹10 shares (at par) for every 4 shares of S Ltd held by outside shareholders. S Ltd's net assets (agreed) are ₹22,00,000; its equity capital is ₹20,00,000.*

**Step 1 — Shares to be settled.** Only the *outside* shareholders (2,00,000 − 40,000 = 1,60,000 shares) receive consideration. H Ltd does not pay itself.

**Step 2 — Shares issued.** 1,60,000 × 3/4 = **1,20,000 shares** × ₹10 = **₹12,00,000**.

**Step 3 — PC.** PC to outside shareholders = **₹12,00,000**. But H Ltd is *acquiring net assets of ₹22,00,000* while also *cancelling its ₹40,000-share investment* (say carried at cost ₹5,00,000 in H's books).

**Step 4 — Goodwill/Capital Reserve.** The comparison is between *what H gives up in total* (PC ₹12,00,000 + book value of investment cancelled ₹5,00,000 = ₹17,00,000) and *net assets acquired* ₹22,00,000:
```
Total cost to H          = 12,00,000 + 5,00,000 = 17,00,000
Net assets acquired      = 22,00,000
Capital Reserve          =  5,00,000
```

> **Why add the investment.** H already "paid" for 20% of S when it bought those 40,000 shares — that historical cost is part of what it has sunk into S. On absorption, that investment is *cancelled* (S no longer exists), so it must be brought into the goodwill/capital-reserve reckoning. Ignoring it is the classic cross-holding error. The entry credits **Investment in S Ltd A/c ₹5,00,000** (removing it) alongside the Business Purchase discharge.

*Examiner tweak:* if H's investment were carried at ₹9,00,000, total cost = 12,00,000 + 9,00,000 = ₹21,00,000 vs NA ₹22,00,000 → still Capital Reserve ₹1,00,000. If carried at ₹11,00,000, total = ₹23,00,000 vs ₹22,00,000 → **Goodwill ₹1,00,000**. The investment's carrying value swings the answer — read it carefully.
...
> **Do not put fictitious assets on the debit of Realisation.** Preliminary expenses, discount on issue, and the debit balance of P&L go **straight to the Equity Shareholders' Account**, not through Realisation. Only *real* assets (whether or not taken over) belong in Realisation.
...
| To Profit & Loss A/c (Dr balance) | XXX | By General Reserve | XXX |
| To Shares in Transferee Co. | XXX | By Profit & Loss A/c (Cr) | XXX |
| To Bank (cash portion) | XXX | By Realisation A/c (profit, if any) | XXX |
| **Total** | **XXX** | **Total** | **XXX** |
...
Use the heads shown in Example 3: Shareholders' funds → Non-current liabilities → Current liabilities; Non-current assets (PPE, Intangibles incl. Goodwill) → Current assets. **Amalgamation Adjustment Reserve** (for statutory reserves under Purchase Method) appears under "Reserves & Surplus" as a *separate negative line* (contra), per AS 14 as amended — disclose it explicitly and reverse it when the statutory reserve is no longer required.

### 6.5 The four-working-note discipline (how to structure a full answer)

A 15-mark amalgamation answer should always present, in order:
1. **Working Note 1 — Purchase Consideration** (state the method, one line per class of shareholder).
2. **Working Note 2 — Net Assets taken over** (agreed values) → hence **Goodwill / Capital Reserve**.
3. **Journal entries** in the transferee's books (or Realisation Account in the transferor's, whichever is asked).
4. **Balance Sheet** in Schedule III format + a one-line **balancing tick**.

Marks are apportioned across these; a clean WN1 and WN2 bank ~40% of the marks even before the balance sheet.
...
```
*Figure 3 — Where amalgamation sits in the syllabus web.*
...
- **Consolidation (AS 21):** In a merger/absorption the transferor *ceases to exist* and there is **one** set of books. In consolidation the subsidiary *survives* as a separate legal entity and you prepare *consolidated* statements — different beast. A useful bridge: cross-holding in amalgamation (Example 5) mirrors "cost of control / capital reserve" in consolidation — both net the acquirer's investment against acquired net assets.
- **Goodwill amortisation (AS 14 / AS 26):** Goodwill on amalgamation is amortised (presumed ≤5 years) — links forward to impairment thinking under AS 28.
- **Purchase consideration discharge** reuses your **Securities Premium** and **share issue** mechanics from company accounts.
- **Debentures & redemption:** taking over the transferor's debentures by issuing fresh debentures reuses the *issue of debentures at par/premium/discount* mechanics.
...
5. **Shares issued at a premium.** Value shares at **issue price** in PC (Net Payments), and route the premium to **Securities Premium** on discharge. Using face value understates PC. But do NOT then add the premium a *second* time on top of PC — the issue price already contains it.
...
8. **Statutory reserves.** Under Purchase Method they must be preserved via an **Amalgamation Adjustment Reserve** — a favourite disclosure question. It is shown as a negative line under Reserves & Surplus (not as an asset, under the amended AS 14).
...
13. **Cross-holding (transferee owns transferor's shares).** Pay only the *outside* shareholders, and net the *cost of the transferee's existing investment* into the goodwill/capital-reserve computation. Both halves are easy to forget (Example 5).
14. **Absorption ≠ purchase automatically.** "New company formed" and "merger accounting" are *independent*. An absorption (existing buyer) can still meet all five conditions and be a merger; an amalgamation (new company) can still be a purchase. Decide by the five-condition test, never by the legal form.
15. **Number of shares vs value of shares.** In share-exchange problems, the *exchange ratio* gives you the *number* of shares; you still multiply by *issue value* (par + premium) to get PC. Stopping at the number, or using par when a premium is stated, understates PC.
16. **Realisation profit sign in the transferor.** Realisation is credited with PC and liabilities-at-book, debited with assets-at-book. If PC exceeds the book net assets, it is a *profit* to the transferor's shareholders — do not confuse it with the transferee's goodwill (measured against *agreed* values). They can point in opposite directions.
...
3. **AS 14's five conditions are the truth-detector.** All five → merger → Pooling. Any breach → purchase → Purchase Method. You don't get to choose. Each condition is a proxy for "did ownership and enterprise genuinely continue?"
4. **Purchase Consideration is what the transferee pays the transferor's *shareholders*** — nothing more. Compute it by **Net Assets** (assets minus liabilities taken over, at agreed values) or **Net Payments** (sum of everything given to shareholders). Debenture-holders and creditors are settled outside PC. The *recipient test* decides everything: shareholder → PC; anyone else → not PC.
5. **Goodwill = PC − Net assets** if positive; a negative gives **Capital Reserve** (Purchase Method only). Pooling adjusts the share-capital difference in reserves instead (revenue reserves first, then capital).
6. **Transferor's books close via a Realisation Account**; every account zeroes out. Fictitious assets bypass Realisation and hit the shareholders directly. **Transferee's books open via a Business Purchase Account**; assets/liabilities come in, PC is discharged in shares/cash/debentures, and the new balance sheet must **balance to the rupee**.
...
**Five merger conditions (all required):** (1) all assets & liabilities transferred; (2) ≥90% equity shareholders (outsiders) become transferee's equity shareholders; (3) consideration to equity shareholders wholly equity shares (cash only for fractions); (4) business continued; (5) book values unchanged except for policy uniformity.
...
- **Net Payments Method:** Cash + Equity shares (at issue value) + Pref. shares + other securities/assets given to shareholders.
- **Intrinsic value / exchange-ratio:** shares to issue = transferor's shares × exchange ratio; PC = shares × issue value.
...
**Cross-holding:** pay only outside shareholders; add the transferee's existing investment (at carrying value) to the cost side when computing goodwill/capital reserve; cancel the investment on discharge.
...
- Transferor: real assets → Realisation (book value); liabilities → Realisation; PC due `Transferee A/c Dr/To Realisation`; reserves & fictitious assets → Equity Shareholders; Realisation profit/loss → Equity Shareholders; final settlement closes all accounts to zero.

**Three names, one toolkit:** Amalgamation (→ new co.), Absorption (→ existing co.), External Reconstruction (→ new co. replacing a battered one). All accounted by AS 14; the *five-condition test*, not the name, picks the method.

**Do-not-confuse:** External reconstruction = new company + PC + Realisation (fictitious losses left behind). **Internal** reconstruction = same company, capital reduction, **no PC, no new company**.

**Balance-sheet checks:** Assets total = Equity + Liabilities total. Reserves carried only under Pooling. Goodwill/Cap. Reserve only under Purchase. Securities Premium on shares issued above par. Statutory reserves preserved via **Amalgamation Adjustment Reserve** (negative line under Reserves & Surplus) under Purchase Method. Amortise goodwill (presumed ≤ 5 years — *verify current ICAI material / AY*).
