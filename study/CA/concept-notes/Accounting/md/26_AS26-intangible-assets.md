...
```mermaid
flowchart TD
    A["Expenditure on an intangible item"] --> B["Identifiable? Controlled? Future benefit?"]
    B -->|No| Z["Expense in P&L"]
    B -->|Yes| C["Probable benefit AND cost measurable reliably?"]
    C -->|No| Z
    C -->|Yes| D["How obtained?"]
    D -->|Purchased or via amalgamation| E["Capitalise at cost or fair value"]
    D -->|Internally generated| F["Goodwill brand masthead customer list?"]
    F -->|Yes| Z
    F -->|No| G["Research phase or Development phase?"]
    G -->|Research| Z
    G -->|Development| H["All six criteria met?"]
    H -->|No| Z
    H -->|Yes| E
```
...
```mermaid
flowchart LR
    A["R&D expenditure in the year"] --> B["Research: always expensed"]
    A --> C["Development: expensed until 6 criteria met"]
    A --> D["Development after criteria met: capitalised"]
    B --> E["Disclose aggregate R&D expensed"]
    C --> E
    D --> F["Shown in intangible asset reconciliation"]
```
