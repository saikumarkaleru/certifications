...
```mermaid
graph TD
  A[Recommendation + target up top] --> B[The thesis: why market is wrong]
  B --> C[Valuation supporting the target]
  C --> D[Catalysts: what closes the gap]
  D --> E[Risks + what would make you wrong]
  E --> F[Financials and model]
```
