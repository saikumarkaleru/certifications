...
```mermaid
flowchart TD
    A["Company wants to accept money"] --> B{"Is it exempted under Rule 2 1 c"}
    B -->|"Yes"| C["Not a deposit - deposit rules do not apply"]
    B -->|"No - it is a deposit"| D{"Is the company a Government company"}
    D -->|"Government company"| E["Governed by RBI / separate limits - special regime"]
    D -->|"No"| F{"Is it an Eligible Company Sec 76"}
    F -->|"Eligible public company net worth 100 cr or turnover 500 cr"| G["May accept from MEMBERS and PUBLIC under Sec 76 - full conditions plus rating"]
    F -->|"Not eligible - ordinary company"| H["May accept only from MEMBERS under Sec 73 2 - conditions apply"]
```
...
**Interest / brokerage:** Cannot exceed the maximum rate prescribed by the **RBI** for NBFC deposits. *Why?* The above-market rate was the Ponzi bait; capping it removes the incentive to lure savers into reckless companies.
...
```mermaid
flowchart LR
    A["Deposit accepted"] --> B["1 Circular to members with financials"]
    B --> C["2 File circular with Registrar 30 days before"]
    C --> D["3 Deposit Repayment Reserve - 20 percent by 30 April"]
    D --> E["4 Provide deposit insurance where notified"]
    E --> F["5 Certify no subsisting default"]
    F --> G["Plus for public - Credit rating and Charge/Trustee if secured"]
```
...
```mermaid
flowchart TD
    A["Board decides to raise deposits"] --> B["Members' resolution GM - special resolution if public"]
    B --> C["Obtain credit rating if eligible company to public"]
    C --> D["Execute deposit trust deed DPT-2 if secured - 7 days before circular"]
    D --> E["Prepare circular / advertisement DPT-1 with financials and rating"]
    E --> F["File DPT-1 with Registrar at least 30 days before issue"]
    F --> G["Issue circular - valid up to 6 months from close of FY or issue date"]
    G --> H["Accept deposits - tenure 6 to 36 months - within limits"]
    H --> I["Provide deposit insurance where notified and create charge if secured"]
    I --> J["Maintain register of deposits - keep 8 years"]
    J --> K["By 30 April each year fund DRRA - 20 percent of next year maturities"]
    K --> L["By 30 June each year file DPT-3 return - position as on 31 March - audited"]
    L --> M["Repay principal and interest on due date"]
```
...
```mermaid
flowchart LR
    A["31 March - snapshot date for return"] --> B["30 April - fund Deposit Repayment Reserve 20 percent"]
    B --> C["30 June - file DPT-3 return of deposits audited"]
    C --> D["Ongoing - repay each deposit on its maturity 6 to 36 months"]
```
...
| Interest cap | Rules | ≤ RBI NBFC max rate | Remove Ponzi bait |
...
| Deposit trustee / charge | Rules / DPT-2 | trust deed 7 days before circular | Watchdog + recourse |
...
| Fraud — personal liability | Sec **75** | unlimited personal liability + Sec 447 | Pierce the veil |
| Contravention penalty | Sec **76A** | company **1 cr–10 cr** (or 2× deposits); officer **25 lakh–2 cr and/or up to 7 yrs** | Social harm = harsh sanction |

> **Confirm the exact current figures (DRRA %, penalty bands, exemption list, insurance status, private-company exemption conditions) against the latest ICAI Study Material and the bare Companies Act 2013 / Deposit Rules before the exam — these have been amended multiple times.**
