...
```mermaid
flowchart TD
  A["Company wants to accept deposit"] --> B{"From public?"}
  B -- "No, from members" --> C{"Private co?"}
  C -- "Yes" --> D["Up to 100% of PUC+FR+SP"]
  C -- "No" --> E["Non-eligible: 35% of PUC+FR+SP"]
  B -- "Yes" --> F{"Eligible co? (NW>=100cr OR TO>=500cr)"}
  F -- "No" --> G["Prohibited"]
  F -- "Yes" --> H["Members 10% + Public 25%; credit rating + DRR"]
```
...
```mermaid
flowchart TD
  A["Declare dividend"] --> B["Out of current/past profits (after depreciation) OR free reserves"]
  B --> C["Deposit in separate bank a/c within 5 days"]
  C --> D["Pay / dispatch warrant within 30 days"]
  D --> E{"Unpaid/unclaimed?"}
  E -- "Yes" --> F["Transfer to Unpaid Dividend A/c within 7 days (after 30-day window)"]
  F --> G["Unclaimed 7 years -> IEPF (with the shares)"]
```
...
```mermaid
flowchart TD
  A["Forex transaction"] --> B{"Alters foreign assets/liabilities?"}
  B -- "No" --> C["Current account txn - Sec 5, freely permitted unless in restricted list"]
  B -- "Yes" --> D["Capital account txn - Sec 6, allowed only if RBI-permitted"]
  C --> E{"Restricted list?"}
  E -- "Sch I" --> F["Prohibited (lottery, football pools)"]
  E -- "Sch II" --> G["Prior CG approval"]
  E -- "Sch III" --> H["Within RBI limits / LRS"]
```
