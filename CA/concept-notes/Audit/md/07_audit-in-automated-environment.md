...
> **Confirm exact wording in ICAI material.** Paraphrase the requirements as above for concept clarity, but reproduce SA numbers and titles precisely in the exam; ICAI marks citation accuracy.
...
2. Put the books inside a computer and the *gap stays* but the *risk relocates*: errors and fraud now hide in **program logic, access rights and data**, repeat **systematically**, and multiply at **huge volume**, often with **no paper trail.**
...
5. Because the risk sits in **processing**, you often cannot audit *around* the black box; you must audit **through** it.
6. The very technology that created the risk gives you the tool to meet it: **CAATs and data analytics** let you re-perform logic and test **100%** of the population — abolishing the sampling compromise that only ever existed because humans couldn't count that high.
7. Finally, **SA 330** makes your procedures respond to what you found. None of this is a new religion — it is the **same risk-based audit**, honestly followed into the machine.

If you can narrate those seven steps, you can derive every requirement in the chapter.

---

## 10. Quick-Revision Sheet

**Why the computer changes the audit (drivers):** loss of visible audit trail · systematic/uniform errors · automated (programmed) controls · unauthorised access & data manipulation · reduced segregation · high volume/speed · **pervasive** IT dependence.

**Anchor standards:** **SA 315** — understand IT environment + controls (ITGCs & application controls) + identify *risks arising from use of IT*. **SA 330** — test ITGCs then application controls if relying; else substantive (CAATs). Also **SA 240** (JE testing), **SA 500** (evidence reliability), **SA 402** (outsourced IT), **SA 620** (IT expert), **SA 230** (documentation).

**Two control layers:**
- **IT General Controls (ITGCs)** — environment-wide; *support other controls*. Domains: **Access security · Change management · Program development/SDLC · IT operations · Backup & recovery.** *Weak ITGCs = cannot rely on ANY automated control.*
- **Application controls** — transaction-level within a process. Types: **input/validation (check digit, range, format, mandatory) · existence/validity · reasonableness/limit · sequence/completeness (control & hash totals) · duplicate · authorisation/matching (3-way match) · configuration · output.** May be automated / manual / IT-dependent manual.

**Keystone:** Test **ITGCs → application controls → reduce substantive testing.** Automated control's **"benefit of consistency"**: test once, cover period — *only because change-management ITGC held.*

**Around vs through:** *Around* = black box, inputs & outputs only, ok for simple/low-volume/visible-trail systems. *Through* = open the box, test processing & programmed controls; needed for complex/high-volume/paper-less/automated systems. *With* = using the computer as a tool.

**CAATs:** **Test data** (dummy txns test programmed controls — don't corrupt live data) · **Integrated Test Facility (ITF)** (dummy entity in live system, reversed) · **Parallel simulation** (auditor's program re-processes real data) · **Embedded audit facility / SCARF** (continuous capture) · **Generalised Audit Software / GAS** (IDEA, ACL — sort, filter, recompute, gaps, duplicates, sample on full population) · **SQL/utility scripts.**
*Use-considerations:* IT skill (SA 620) · availability/compatibility of tools · impracticability of manual tests · effectiveness & efficiency · timing (transient data) · **data integrity / don't corrupt live systems** · cost-benefit.

**Data analytics uses:** 100% population testing · **journal-entry testing** (weekend/odd-user/round-sum/just-below-limit/unusual account) · 3-way match · duplicate & gap detection · ageing & recomputation · Benford's Law. *Output = leads to investigate, not conclusions; relies on complete/reliable data (→ ITGCs).*
