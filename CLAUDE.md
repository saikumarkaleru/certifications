...
There must be **exactly one consolidated PDF per subject** — the six `*_FULL.pdf` files in `study/CA/concept-notes/`:
...
- After generating anything, rebuild the affected subject PDF(s) with `scratchpad build_subject_pdf.py` and confirm the count under `study/CA/concept-notes/*.pdf` is still exactly **6**.
